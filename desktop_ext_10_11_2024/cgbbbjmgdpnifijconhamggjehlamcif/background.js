...
chrome.alarms.onAlarm.addListener(function (alarm) {
  ...
  if (appConfig.debugAnalytics) {
    ...
    Analytics.sendEvent({
      category: 'DEBUG',
      action: 'Alarm Listener Called',
      label: alarm.name.split('|')[1] + ' ' + latencyInMs + ' ms',
      value: latencyInMs
    });
  }
    ...
    getDeviceId(function (deviceId) {
      ...
      if (deviceId) {
        ...
        if (window.navigator.onLine) {
          ...
          getUserProfile(deviceId, 5, function (userInfo) {

            setLocalVars({
              domain: parseDomainFromUserInfo(userInfo),
              userId: userInfo.id,
              userEmail: userInfo.email
            }, () => {
            });

            if (!vars.userJWT || vars.userJWT === 'NOT_SET' || !vars.licenseTier || !vars.licenseTimestamp || (new Date().getTime() - parseInt(vars.licenseTimestamp, 10) > appConfig.checkRateInSeconds.licenseRefresh * 1000)) {
...
function getUserProfile(deviceId, numTries = 5, callback) {
  ...
  // add backoff and retry if getProfileUserInfo fails
  chrome.identity.getProfileUserInfo({accountStatus:'ANY'}, (userInfo) => {
    if (numTries > 0 && (!userInfo || !userInfo.id || !userInfo.email)) {
      return setTimeout(() => getUserProfile(deviceId, numTries - 1, callback), 1000);
    }
      ...
      if (responseCode == 501) {
        ...
        if (appConfig.debugAnalytics) {
          Analytics.sendEvent({
            ...
            action: appConfig.api.getCustomerTier + ' onRequestError',
            label: '501 Bad Gateway',
            value: 501,
            domain: vars.domain,
            licenseTier: vars.licenseTier
          });
        ...
        setLocalVars({
          ...
          userJWT: responseObject.userJWT ?? null
        });
...
function onRequestError(requestType, label) {
  if (appConfig.debugAnalytics) {
    Analytics.sendEvent({
      category: 'Error',
      action: requestType + ' onRequestError',
      label: label,
      domain: vars.domain,
      licenseTier: vars.licenseTier
    });
      ...
      case 304:
        ...
        if (params.requestType && params.requestType === 'POST') {
          ...
          if (appConfig.trackAnalytics) {
            Analytics.sendEvent({
              category: 'Gopher Buddy API',
              action: appConfig.api.deviceLookup,
              value: responseCode,
              label: responseCode + ' ' + JSON.stringify(responseObject),
              domain: vars.domain,
              licenseTier: vars.licenseTier
            });
          }
  ...
  chrome.alarms.getAll(function (alarms) {
    ...
    if (appConfig.debugAnalytics) {
      Analytics.sendEvent({
        category: 'Extension',
        action: 'Killswitch',
        label: vars.deviceId + ' >> ' + alarms.length,
        domain: vars.domain,
        licenseTier: vars.licenseTier,
        deviceId: vars.deviceId
      });
    ...
    chrome.storage.local.get(['lastUpdateNotification'], function (data) {
      ...
      if (vars.domainSettings.allowChromeOsUpdateReminders) {
        ...
        Analytics.sendEvent({
          category: 'Notification',
          action: 'Prevented',
          label: 'Version pop-up notice blocked due to domain settings',
          previousVersion: vars.deviceVersion,
          newVersion: vars.available,
          domain: vars.domain,
          licenseTier: vars.licenseTier
...
async function getChromeOsVersion() {
  ...
  if (navigator.userAgentData) {
    userAgent = await navigator.userAgentData.getHighEntropyValues(['uaFullVersion'])
    version = userAgent.uaFullVersion
  } else {
    userAgent = navigator.userAgent; // may not contain the minor version
    const verExp = new RegExp('Chrome\/([0-9.]+)');
    version = verExp.exec(userAgent)[1];
  }
