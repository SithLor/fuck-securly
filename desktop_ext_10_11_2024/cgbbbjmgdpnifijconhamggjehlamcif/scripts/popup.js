  ...
  .controller('PopupCtrl', function ($q, $scope, $window, $timeout) {
    ...
    _this.openDeviceDetails = function () {
      ...
      try {
        ...
        pr.push(_this.vars.userJWT ?? 'JWT_NOT_SET');
        ...
        try {
          ...
          chrome.enterprise.deviceAttributes.getDirectoryDeviceId(function(deviceId) {
           ...
           // deviceIdPromise.reject(e.message); // TODO: **PRE_DEPLOY** ENABLE FOR DEPLOYED ENV
           deviceIdPromise.resolve('65d8d367-3256-44bb-a047-805d0028c32a'); // TODO: **PRE_DEPLOY** ENABLE FOR LOCAL DEV
        }
      
        $q.all(pr).then(function(resp) {
          var userToken = resp[0];
          var email = resp[1].email
