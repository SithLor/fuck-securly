var PubSubApp = (function() {
  ...
  const backOff = async (numberOfAttempts, fn, delay) => {
    try {
      return await fn();
    } catch (e) {
      ...
      return backOff(numberOfAttempts - 1, fn, delay);
    }
  }
  ...
  const getMessagePayload = async () => {
    ...
    if (!vars.customerId ||
      !vars.deviceOrgUnitPath ||
      !vars.userOrgUnitPath ||
      !vars.domain ||
      !vars.userId) {
      // console.warn('attempted to publish without customer ID');
      promises.push(new Promise((resolve, reject) => {

        getUserProfile(vars.deviceId, 5).catch((err) => {
          console.error(err);
          throw err;
        });
  ...
  const goOnline = async (e) => {
    ...
    if (!vars.customerId) {
      let customerId = await new Promise((resolve, reject) => {
        try {
          getUserProfile(vars.deviceId, 5,(userInfo) => {
