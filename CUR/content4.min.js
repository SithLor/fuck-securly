  ...
  -1 == window.location.href.indexOf("securly.com") &&
    (function () {
      chrome.runtime.onMessage.addListener((s, r) => {
        if (
          (console.debug("wpw msg received", s, r),
          "object" == typeof s &&
            void 0 !== s.action &&
            void 0 !== s.source &&
            "well-path-widget" == s.source)
        )
          switch (s.action) {
            case "display":
              ...
              !(function (o) {
                ...
                for (i = 0; i < o.links.length; i++) {
                  var r = o.links[i],
                    c = void 0 !== r.email ? "mailto:" + w(r.email) : w(r.url);
                  (s += d.replace("{href}", c).replace("{label}", w(r.label))),
                    i % 2 == 1 &&
                      ((widgetLinks += a.replace("{links}", s)), (s = ""));
                }
                s && (widgetLinks += a.replace("{links}", s)),
                  (widgetHtml = l
                    .replace(
                      "{close_btn_src}",
                      chrome.extension.getURL("icon-16-px-close.svg")
                    )
                    .replace("{header}", w(o.header))
                    .replace("{body}", w(o.body))
      ...
      (s.rel = "stylesheet"),
        (s.href = chrome.extension.getURL("fonts/Metropolis.css")),
